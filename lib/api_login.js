 ...
 * 用户数据的解密
 *
 * 用于getUserInfo&getPhoneNumber接口数据的解密
 *
 * @param {*} session_key
 * @param {*} payload
 ...
 * @param {*} callback
 */
exports.decryptPayload = function (session_key, payload, callback) {
  const decoded = decryptPayload(session_key, payload);
  if (decoded.watermark.appid !== this.appid) {
    return callback(new Error('Illegal Buffer'));
  }
